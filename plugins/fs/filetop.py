...
parser = argparse.ArgumentParser(
    ...
    epilog=examples)
# action从store_true改为store_false，这样就可以输出所有类型的文件
parser.add_argument("-a", "--all-files", action="store_false",
    help="include non-regular file types (sockets, FIFOs, etc)")
# action从store_true改为store_false，这样不用指定该参数也能不清屏输出
parser.add_argument("-C", "--noclear", action="store_false",
    help="don't clear the screen")
# 取消输出数量的限制，输出所有的事件
# parser.add_argument("-r", "--maxrows", default=99999999,
...
data_struct = {"measurement": 'filetop',
               "tags": ['glob'],
               "fields": ['time', 'pid', 'comm','reads', 'writes', 'read_KB','write_KB', 'T', 'file']}
...
while 1:
    ...
    for k, v in reversed(sorted(counts.items(),
        ...
        #     k.type.decode('utf-8', 'replace'), name))
         # write to influxdb
        test_data = lmp_data(datetime.now().isoformat(),'glob',k.pid,
            k.comm.decode('utf-8', 'replace'), v.reads, v.writes,
            v.rbytes / 1024, v.wbytes / 1024,
            k.type.decode('utf-8', 'replace'), name)
        # print(test_data)
