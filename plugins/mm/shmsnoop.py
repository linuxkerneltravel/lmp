...
data_struct = {"measurement":'shmsnoop',
               "time":[],
               "tags":['pid',],
               "fields":['comm','sys','ret','Key','Size','shmflg','shmid','shmaddr','cmd','buf']}
...
def print_event(cpu, data, size):
    ...
    if event.sys == SYS_SHMGET:
        ...
        test_data = lmp_data(event.id >> 32,event.comm.decode(),sys_name(event.sys),'0x{:02X}'.format(event.ret),'0x{:02X}'.format(event.key),str(event.size),shmflg_str(event.shmflg, shmat_flags),'','','','')
    ...
    if event.sys == SYS_SHMAT:
        ...
        test_data = lmp_data(event.id >> 32,event.comm.decode(),sys_name(event.sys),'0x{:02X}'.format(event.ret),'','',shmflg_str(event.shmflg, shmat_flags),'0x{:02X}'.format(event.shmid),'0x{:02X}'.format(event.shmaddr),'', '')
        
    if event.sys == SYS_SHMDT:
        # print("shmaddr: 0x%lx" % (event.shmaddr))
        test_data = lmp_data(event.id >> 32,event.comm.decode(),sys_name(event.sys),'0x{:02X}'.format(event.ret),'','','','', '0x{:02X}'.format(event.shmaddr),'', '')
        
    if event.sys == SYS_SHMCTL:
        # print("shmid: 0x%lx, cmd: %lu, buf: 0x%x" % (event.shmid, event.cmd, event.buf))
        test_data = lmp_data(event.id >> 32,event.comm.decode(),sys_name(event.sys),'0x{:02X}'.format(event.ret),'','','','0x{:02X}'.format(event.shmid), '','0x{:02X}'.format(event.cmd),'0x{:02X}'.format(event.buf))
