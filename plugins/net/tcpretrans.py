...
parser = argparse.ArgumentParser(
    ...
    epilog=examples)
parser.add_argument("-l", "--lossprobe", action="store_false",
...
data_struct = {"measurement":'tcpretrans',
               "time":[],
               "tags":['glob',],
               "fields":['time','pid','ip','laddr','lport','T','daddr','dport','state']}
...
def print_ipv4_event(cpu, data, size):
    ...
    # write to influxdb
    test_data = lmp_data(datetime.now().isoformat(),'glob',event.pid, event.ip,
