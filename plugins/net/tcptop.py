...
data_struct = {"measurement":'tcptop',
               "time":[],
               "tags":['glob',],
               "fields":['time','pid','comm','laddr','raddr','rx_KB','tx_KB']}
...
while i != args.count and not exiting:
    ...
    for k, (send_bytes, recv_bytes) in sorted(ipv4_throughput.items(),
        ...
        # write to influxdb
        test_data = lmp_data(datetime.now().isoformat(),'glob',k.pid,pid_to_comm(k.pid), k.laddr + ":" + str(k.lport),
            k.daddr + ":" + str(k.dport),
            int(recv_bytes / 1024), int(send_bytes / 1024))
        #print(test_data)
