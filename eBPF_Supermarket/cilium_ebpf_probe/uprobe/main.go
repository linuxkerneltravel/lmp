...
func main() {
	...
	up, err := ex.UprobeOffset
	up, err := ex.Uprobe(symbol, objs.UprobeAhttpMain, &UprobeOptions{Offset: 0x0000})
	if err != nil {
	...
	for {
		...
		log.Printf("%s:%s return value: ", binPath, symbol)
		log.Printf("Method: %s, URI: %s, Status: %s, ReturnMsg: %s\n",
			event.Method[:], event.Uri[:], event.StatusCode, event.Msg[:])
