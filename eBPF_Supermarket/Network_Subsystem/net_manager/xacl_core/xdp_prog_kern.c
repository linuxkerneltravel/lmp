...
struct {
	...
	__type(key, ETH_ALEN);
	__type(value, __u32);
	__uint(max_entries, MAX_SRC_MACS);
} src_macs SEC(".maps");
...
SEC("xdp")
int filter_ethernet_pass(struct xdp_md *ctx)
{
	xdp_act action = XDP_DROP; 
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
...
SEC("xdp")
int filter_ethernet_drop(struct xdp_md *ctx)
{
...
/*redirect packets to other mac addresses*/
SEC("xdp")
