...
int BPF_KPROBE(ip_rcv_core, struct sk_buff *skb) {
    ...
    struct packet_tuple pkt_tuple = {0};//定义一个packet_tuple结构体变量pkt_tuple并初始化
    get_pkt_tuple(&pkt_tuple, ip, tcp);
    struct ktime_info *tinfo;
    tinfo = bpf_map_lookup_elem(&timestamps, &pkt_tuple);//在timestamps映射中查找元素pkt_tuple
    if (tinfo == NULL) {
        return 0;
    }
...
//根据传入的数据包提取关键信息（如IP和TCP头部信息），并将这些信息与其他元数据（如套接字信息和错误标识）一同存储到BPF ring buffer中
...
int BPF_KPROBE(tcp_validate_incoming, struct sock *sk, struct sk_buff *skb) {
    ...
    if (sk == NULL || skb == NULL)
        return 0;
    struct conn_t *conn = bpf_map_lookup_elem(&conns_info, &sk);//BPFmap查找与套接字sk关联的信息
    if (conn == NULL) {
        return 0;
    }
    ...
    //  invalid seq
    u16 family = BPF_CORE_READ(sk, __sk_common.skc_family);//获取套接字的地址族就是获得当前ip协议
    struct packet_tuple pkt_tuple = {0};
    if (family == AF_INET) {
        struct iphdr *ip = skb_to_iphdr(skb);
        struct tcphdr *tcp = skb_to_tcphdr(skb);
...
int BPF_KPROBE(tcp_sendmsg, struct sock *sk, struct msghdr *msg, size_t size) {
    ...
    if (family == AF_INET) {
        ...
        u32 snd_nxt = BPF_CORE_READ(tcp_sk(sk), snd_nxt);//tcp要发送的下一个字节序列号
        u32 rcv_nxt = BPF_CORE_READ(tcp_sk(sk), rcv_nxt);//tcp接收的下一个字节的期望序列号
        pkt_tuple.seq = snd_nxt;
        pkt_tuple.ack = rcv_nxt;

        tinfo = (struct ktime_info *)bpf_map_lookup_or_try_init(
            &timestamps, &pkt_tuple, &zero);//timestamps的BPF map保存数据包与时间戳的映射
        if (tinfo == NULL) {
            return 0;
        }
        tinfo->tcp_time = bpf_ktime_get_ns() / 1000;
...
//sk套接字中获取源端口号
        ...
        u32 snd_nxt = BPF_CORE_READ(tcp_sk(sk), snd_nxt);//发送的下一个字节序列号
        u32 rcv_nxt = BPF_CORE_READ(tcp_sk(sk), rcv_nxt);//期望接收的下一个字节序列号
        pkt_tuple.seq = snd_nxt;
        pkt_tuple.ack = rcv_nxt;
...
int BPF_KPROBE(ip_queue_xmit, struct sock *sk, struct sk_buff *skb) {
    ...
    if (family == AF_INET) {
        ...
        tinfo->ip_time = bpf_ktime_get_ns() / 1000;
        bpf_printk("%llu   2",tinfo->ip_time);
    }
    return 0;
};
