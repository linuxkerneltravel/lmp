...
int __udp_send_skb(struct sk_buff *skb)
{
    ...
    struct ktime_info *tinfo, zero = {0};
    bpf_printk("udp_send_skb%d %d %d %d",pkt_tuple.saddr,pkt_tuple.daddr,pkt_tuple.sport,pkt_tuple.dport);
    tinfo = (struct ktime_info *)bpf_map_lookup_or_try_init(&timestamps,
                                                         &pkt_tuple, &zero);
    if (tinfo == NULL) {
        return 0;
...
int __ip_send_skb(struct sk_buff *skb)
{
    ...
    struct packet_tuple pkt_tuple = {0};
    get_udp_pkt_tuple(&pkt_tuple, ip, udp);
    bpf_printk("ip_send_skb%d %d %d %d",pkt_tuple.saddr,pkt_tuple.daddr,pkt_tuple.sport,pkt_tuple.dport);
    struct ktime_info *tinfo, zero = {0};
    tinfo = bpf_map_lookup_elem(&timestamps, &pkt_tuple);
    if (tinfo == NULL) {
        return 0;
...
static __always_inline int process_dns_packet(struct sk_buff *skb, int rx) {
    ...
    struct dns_information *message = bpf_ringbuf_reserve(&dns_rb, sizeof(*message), 0);
    if (!message)
        return 0;

    //计算dns数据开始偏移  传输层头部的位置加上 UDP 头部的大小
    u32 dns_offset = BPF_CORE_READ(skb, transport_header) + sizeof(struct udphdr);
