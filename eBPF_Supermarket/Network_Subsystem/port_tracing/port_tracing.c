...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    int pid=d->pid;
	int sum=d->sum;
    unsigned int sadder=d->sadder;
    unsigned int dadder=d->dadder;
    unsigned int port=d->port;
	if(!local){
		if((dadder & 0x0000a8c0) == 0x0000a8c0 || (dadder & 0x0000007f) == 0x0000007f)
			return 0;
	}
    printf("%-22d %-22s %-22d  %-22s %-22s %-22d \n",pid,d->comm,sum,inet_ntop(AF_INET, &sadder, s_str, sizeof(s_str)),inet_ntop(AF_INET, &dadder, d_str, sizeof(d_str)),port);
    return 0;
}
...
int main(int argc, char **argv)
{
	...
	err = port_tracing_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load and verify BPF skeleton\n");
		goto cleanup;
	}
    /* Attach tracepoints */
	err =port_tracing_bpf__attach(skel);
	if (err) {
	...
	printf("%-22s %-22s  %-22s  %-22s  %-22s  %-22s \n" ,
		"pid","comm","sum","sadder","dadder" ,"port");
		//printf("%-22s %-22s\n","pid" ,"sum" );
	while (!exiting) {
...
cleanup:
	...
	port_tracing_bpf__destroy(skel);
