...
# # 维度判定阈值（标准化后的绝对 z 分数阈值）
# DIM_Z_THRESHOLD = float(os.environ.get("AIMS_DIM_Z", "2.5"))
# # 模型异常比例（可调高以更敏感，比如 0.15）
# CONTAMINATION = float(os.environ.get("AIMS_CONTAMINATION", "0.05"))
# # 异常时若未超过阈值，回退显示的前 K 个维度数
# DIM_TOPK = int(os.environ.get("AIMS_DIM_TOPK", "2"))
# # 正常状态连续 N 次后自动关闭 NET 打印
# NET_OFF_AFTER_NORMAL = int(os.environ.get("AIMS_NET_OFF_AFTER_NORMAL", "3"))
...
#         df["timestamp"] = pd.to_datetime(today + " " + df["timestamp"], errors="coerce")
#     return df
...
# ===== 路径设置 =====
DATA_DIR = "./data"
CPU_FILE = os.path.join(DATA_DIR, "cpu_metrics.csv")
MEM_FILE = os.path.join(DATA_DIR, "mem_metrics.csv")
IO_FILE  = os.path.join(DATA_DIR, "io_metrics.csv")
NET_FILE = os.path.join(DATA_DIR, "net_metrics.csv")

all_features = ["cpu_usage","cpu_wait","mem_usage","io_tps","io_read","io_write","net_in","net_out"]

# ===== 只读最后一行的工具函数 =====
def read_last_row(path):
    try:
        with open(path, "rb") as f:
            f.seek(-200, os.SEEK_END)  # 只读文件最后 200 字节（足够覆盖一行）
...
def build_seed_features():
    ...
    df_cpu["cpu_usage"] = df_cpu["us"] + df_cpu["sy"]
    df_cpu["cpu_wait"]  = df_cpu["wa"]
    cpu_feat = df_cpu[["cpu_usage","cpu_wait"]]

    df_mem = pd.read_csv(MEM_FILE)
    df_mem["mem_usage"] = df_mem["used_MB"] / df_mem["total_MB"].replace(0, np.nan)
    mem_feat = df_mem[["mem_usage"]]

    df_io = pd.read_csv(IO_FILE)
    io_feat = df_io.groupby("timestamp").agg({
        "tps":"sum","kB_read_s":"sum","kB_wrtn_s":"sum"
    }).reset_index().rename(columns={
        "tps":"io_tps","kB_read_s":"io_read","kB_wrtn_s":"io_write"
    })[["io_tps","io_read","io_write"]]

    df_net = pd.read_csv(NET_FILE)
    net_feat = df_net.groupby("timestamp").agg({
        "rx_bytes_s":"sum","tx_bytes_s":"sum"
    }).reset_index().rename(columns={"rx_bytes_s":"net_in","tx_bytes_s":"net_out"})[["net_in","net_out"]]

    df = pd.concat([cpu_feat, mem_feat, io_feat, net_feat], axis=1)
    return df.fillna(0)

# ===== 初始化模型 =====
seed_df = build_seed_features()
scaler = StandardScaler()
X_seed = scaler.fit_transform(seed_df[all_features])
...
while True:
    try:
        # 只取 CSV 的最后一行
        last_cpu = pd.read_csv(CPU_FILE).tail(1)
        last_mem = pd.read_csv(MEM_FILE).tail(1)
        last_io  = pd.read_csv(IO_FILE).tail(1)
        last_net = pd.read_csv(NET_FILE).tail(1)

        # 构建一行特征
        row = {
            "cpu_usage": last_cpu["us"].iloc[0] + last_cpu["sy"].iloc[0],
            "cpu_wait":  last_cpu["wa"].iloc[0],
            "mem_usage": last_mem["used_MB"].iloc[0] / max(last_mem["total_MB"].iloc[0], 1),
            "io_tps":    last_io["tps"].sum(),
            "io_read":   last_io["kB_read_s"].sum(),
            "io_write":  last_io["kB_wrtn_s"].sum(),
            "net_in":    last_net["rx_bytes_s"].sum(),
            "net_out":   last_net["tx_bytes_s"].sum()
        }

        X_cur = scaler.transform([list(row.values())])
        y_pred = model.predict(X_cur)[0]   # -1 异常, 1 正常
        print(f"[{time.strftime('%H:%M:%S')}] 状态: {'异常' if y_pred == -1 else '正常'} 特征: {row}")
