...
int BPF_PROG(sched_switch, bool preempt, struct task_struct *prev, struct task_struct *next) {
    ...
    bpf_map_update_elem(&threshold_schedule, &sched_key, &proc_schedule, BPF_ANY);
    sum_schedule = bpf_map_lookup_elem(&sys_schedule, &sched_key);
    if (!sum_schedule) {
        struct sum_schedule sum_schedule = {};
        ...
        bpf_map_update_elem(&sys_schedule, &sched_key, &sum_schedule, BPF_ANY);
    } else {
        sum_schedule->sum_count++;
        sum_schedule->sum_delay += delay;
        if (delay > sum_schedule->max_delay) {
