...
void cleanup_skeleton(void *skel, void (*destroy_fn)(void *))
{
    if (skel)
        destroy_fn(skel);
}
...
int main(int argc, char **argv)
{
    ...
    if (env.CS_DELAY)
    {
        ...
        rb = ring_buffer__new(bpf_map__fd(cs_skel->maps.rb), handle_event, NULL, NULL);
        if (!rb) {
            err = -1;
        ...
        rb = ring_buffer__new(bpf_map__fd(sc_skel->maps.rb), syscall_delay_print, NULL, NULL);
        if (!rb) {
            err = -1;
            fprintf(stderr, "Failed to create ring buffer\n");
            goto cleanup;        
        ...
        rb = ring_buffer__new(bpf_map__fd(mq_skel->maps.rb), mq_event, NULL, NULL);
        if (!rb) {
            err = -1;
            fprintf(stderr, "Failed to create ring buffer\n");
            goto cleanup;
...
cleanup:
    ...
    cleanup_skeleton(cs_skel, (void (*)(void *))cs_delay_bpf__destroy);
    cleanup_skeleton(sar_skel, (void (*)(void *))sar_bpf__destroy);
    cleanup_skeleton(sc_skel, (void (*)(void *))sc_delay_bpf__destroy);
    cleanup_skeleton(preempt_skel, (void (*)(void *))preempt_bpf__destroy);
    cleanup_skeleton(sd_skel, (void (*)(void *))schedule_delay_bpf__destroy);
    cleanup_skeleton(mq_skel, (void (*)(void *))mq_delay_bpf__destroy);
    return err < 0 ? -err : 0;
}
