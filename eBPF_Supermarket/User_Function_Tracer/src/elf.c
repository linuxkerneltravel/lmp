...
bool elf_rela_entry_next(struct elf_rela_entry* elf_e, struct elf_section* elf_s) {
  if (elf_s->shdr.sh_type != SHT_RELA || elf_e->i >= elf_e->nentries) return false;
...
bool elf_versym_entry_next(struct elf_versym_entry* elf_e, struct elf_section* elf_s) {
  if (elf_s->shdr.sh_type != SHT_GNU_versym || elf_e->i >= elf_e->num) return 0;
...
bool elf_verdef_entry_next(struct elf_verdef_entry* elf_e, struct elf_section* elf_s) {
  if (elf_s->shdr.sh_type != SHT_GNU_verdef || elf_e->i >= elf_s->shdr.sh_info) return false;
  if (elf_e->i) elf_e->offset += elf_e->verdef.vd_next;
...
bool elf_verneed_entry_next(struct elf_verneed_entry* elf_e, struct elf_section* elf_s) {
  if (elf_s->shdr.sh_type != SHT_GNU_verneed || elf_e->i >= elf_s->shdr.sh_info) return false;
