...
static const struct argp_option opts[] = {
    ...
    {"percent-self", OPT_PERCENT_SELF, NULL, 0, "Show percentage of self function time in report",
     0},
    {"percent-total", OPT_PERCENT_TOTAL, NULL, 0,
     "Show percentage of total function time in report", 0},
...
static error_t parse_arg(int key, char *arg, struct argp_state *state) {
  ...
  switch (key) {
    ...
    case OPT_FORMAT:  // --format
      if (!strncmp(arg, "summary", 7)) env.format = SUMMARY;
      if (!strncmp(arg, "chrome", 6))
        env.format = CHROME;
    ...
    case 'p':  // -p/--pid
      ...
      break;
    case OPT_PERCENT_SELF:
      env.percent_self = true;
      break;
    case OPT_PERCENT_TOTAL:
    ...
    case OPT_SAMPLE_TIME:
      ...
      if (env.sample_time_ns == 0) {
        ERROR("The parameter for --sample_time should be greater than \"0ns\"");
        return 1;
      }
      break;
    case OPT_TID:  // --tid
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
  ...
  user_record.krecord = *((struct kernel_record *)data);
