...
#include <signal.h>
#include "open.skel.h"	//包含了BPF字节码和相关的管理函数
#include "open.h"
...
static int handle_event(void *ctx, void *data,unsigned long data_sz)
{
	...
	printf("pid:%d uid:%d time:%lu fd:%d comm:%s\n",e->pid,e->uid,e->ts,e->fd,e->comm);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	/* 设置环形缓冲区轮询*/
	rb = ring_buffer__new(bpf_map__fd(skel->maps.rb), handle_event, NULL, NULL);	//ring_buffer__new() API锛屽厑璁稿湪涓嶄娇鐢ㄩ澶栭€夐」鏁版嵁缁撴瀯涓嬫寚瀹氬洖璋?
	if (!rb) {
		err = -1;
	...
	/* 处理事件 */
	while (!exiting) {
		err = ring_buffer__poll(rb, 100 /* timeout, ms */);		//ring_buffer__poll(),杞鎵撳紑ringbuf缂撳啿鍖恒€傚鏋滄湁浜嬩欢锛宧andle_event鍑芥暟浼氭墽琛?
