...
# arguments
examples = """examples:
    ./read          # trace file read info
    ./read -c       # trace TOP10 info
"""
parser = argparse.ArgumentParser(
    description="Trace file read info",
...
BPF_HASH(read_info, u64, struct val_t);
BPF_HASH(rettmp, u64, struct tmp_t);
BPF_HASH(fdtmp, u64, struct tmp_t);

int entry_vfs_read(struct pt_regs *ctx, struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
    struct val_t val = {};
    ...
    if (bpf_get_current_comm(&val.comm, sizeof(val.comm)) == 0)
    {
        ...
        read_info.update(&ts, &val);
    }

    return 0;
}

int entry_ksys_read(struct pt_regs *ctx, unsigned int fd, const char __user *buf, size_t count)
{
    struct tmp_t tmp= {};
    u64 id = bpf_get_current_pid_tgid(); 
    u32 self = id >> 32;
...
int exit_vfs_read(struct pt_regs *ctx)
{
    struct tmp_t tmp= {};
    u64 id = bpf_get_current_pid_tgid(); 
    u32 self = id >> 32;
...
read_info = b.get_table("read_info")
...
def print_info():
    ...
    while True:
        try:
            sleep(interval)
            for k, v in sorted(read_info.items(), key=lambda read_info:read_info[0].value):
