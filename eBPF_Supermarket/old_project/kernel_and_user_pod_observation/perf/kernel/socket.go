...
import (
	...
	"github.com/linuxkerneltravel/lmp/eBPF_Supermarket/kernel_and_user_pod_observation/bpf/tcpaccept"
	"github.com/linuxkerneltravel/lmp/eBPF_Supermarket/kernel_and_user_pod_observation/bpf/tcpconnect"
...
func updateVecMetric(vec *prometheus.SummaryVec, labels map[string]string, value float64) {
	if value < 0 {
		return
	}
		...
		case v1 := <-acceptChan:
			if slices.Contains(sidecarPidList, v1.Pid) { // got SidecarAcceptEvent
				...
				if ok == false || (pair.Ok || pair.Ok == false && pair.SidecarConnect == sice) {
					...
					updateVecMetric(sidecarTime, map[string]string{"pid": strconv.Itoa(v1.Pid), "podName": podName}, float64(pair.SidecarConnect.Time-pair.SidecarAccept.Time))
				...
				if ok == false || pair.Ok || (pair.Ok == false && pair.SidecarConnect == sice) {
					...
					updateVecMetric(sidecarToServiceTime, map[string]string{"pid": strconv.Itoa(v1.Pid), "podName": podName}, float64(pair.ServiceAccept.Time-pair.SidecarConnect.Time))
		...
		case v2 := <-connectChan:
			if slices.Contains(sidecarPidList, v2.Pid) { // got SidecarConnectEvent
				...
				if ok == false || pair1.Ok || (pair1.Ok == false && pair1.SidecarAccept == siae) {
					...
					updateVecMetric(sidecarTime, map[string]string{"pid": strconv.Itoa(v2.Pid), "podName": podName}, float64(pair1.SidecarConnect.Time-pair1.SidecarAccept.Time))
				...
				if ok == false || pair2.Ok || (pair2.Ok == false && pair2.ServiceAccept == seae) {
					...
					updateVecMetric(sidecarToServiceTime, map[string]string{"pid": strconv.Itoa(v2.Pid), "podName": podName}, float64(pair2.ServiceAccept.Time-pair2.SidecarConnect.Time))
