...
typedef struct data {
    ...
    u64 total_read;				//stats_array key:1  value:保存read的次数，下面同理
    u64 total_write;			//stats_array key:2
    u64 total_fsync;			//stats_array key:3
    u64 total_open;				//stats_array key:4
    u64 total_create;			//stats_array key:5
    u64 apcl;					//cachestat key:1
    u64 mpa;					//cachestat key:2
    u64 apd;					//cachestat key:3
    u64 mbd;					//cachestat key:4
} data_t;
...
int trace(struct pt_regs *ctx, struct task_struct *prev)
{
    ...
    int key_open = 4;
    int key_create = 5;
    u64 key_apcl = 1;			//cachestat key:1
    u64 key_mpa = 2;			//cachestat key:2
    u64 key_apd = 3;			//cachestat key:3
    u64 key_mbd = 4;			//cachestat key:4
...
//------------------------------------------------
	...
	if (cur-*time_old >= 1000000000) {
        ...
        u64 * fs = stats.lookup(&key_read);
        if (fs) data.total_read = *fs;
        fs = stats.lookup(&key_write);
        if (fs) data.total_write = *fs;
        fs = stats.lookup(&key_fsync);
        if (fs) data.total_fsync = *fs;
        fs = stats.lookup(&key_open);
        if (fs) data.total_open = *fs;
        fs = stats.lookup(&key_create);
        if (fs) data.total_create = *fs;

        fs = cachestat.lookup(&key_apcl);
        if (fs) data.apcl = *fs;
        fs = cachestat.lookup(&key_mpa);
        if (fs) data.mpa = *fs;
        fs = cachestat.lookup(&key_apd);
        if (fs) data.apd = *fs;
        fs = cachestat.lookup(&key_mbd);
        if (fs) data.mbd = *fs;        
...
//下面四个函数实现页缓存命中率，页缓存命中率使用
int do_count_apcl(struct pt_regs *ctx) {
	//key为1代表apcl
    u64 ip = 1;
    cachestat.increment(ip); // update counter
    return 0;
}
