    ...
    try:
        if event.type == EventType.EVENT_MOUNT:
            ...
            if parent:
                ...
                test_data = lmp_data(datetime.now().isoformat(),'glob',event.union.enter.comm,event.pid,event.tgid,event.union.enter.mnt_ns,call)
...
data_struct = {"measurement":'mountsnoop',
               "time":[],
               "tags":['glob',],
               "fields":['time','COMM','PID','TID','MNT_NS','CALL']}
