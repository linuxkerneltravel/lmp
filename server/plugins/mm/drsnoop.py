...
data_struct = {"measurement":'drsnoop',
               "time":[],
               "tags":[],
               "fields":['time','COMM','PID','LAT_ms','PAGES']}
...
def print_event(cpu, data, size):
    ...
    test_data = lmp_data(datetime.now().isoformat(),event.name.decode('utf-8', 'replace'),event.id >> 32,float(event.delta) / 1000000,event.nr_reclaimed)
