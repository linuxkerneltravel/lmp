...
parser.add_argument("-L", "--lport", action="store_false",
...
data_struct = {"measurement":'tcpconnlat',
               "time":[],
               "tags":['glob',],
               "fields":['time','pid','comm','ip','saddr','lport','daddr','dport','lat_ms']}
...
def print_ipv4_event(cpu, data, size):
    ...
    if args.lport:
        ...
        test_data = lmp_data(datetime.now().isoformat(),'glob',event.pid,
            event.task.decode('utf-8', 'replace'), event.ip,
            inet_ntop(AF_INET, pack("I", event.saddr)), 0,
            inet_ntop(AF_INET, pack("I", event.daddr)), event.dport,
            float(event.delta_us) / 1000)
         #print(test_data)
        write2db(data_struct, test_data, influx_client, DatabaseType.INFLUXDB.value)
