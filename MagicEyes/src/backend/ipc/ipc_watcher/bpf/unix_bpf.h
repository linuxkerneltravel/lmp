...
// 拷贝修改自 /net/unix/unix_bpf.c， kernel version >= 5.15 会有这个文件
...
static __always_inline int unix_msg_wait_data(struct sock *sk, struct sk_psock *psock,
			      long timeo)
{
...
static __always_inline int __unix_recvmsg(struct sock *sk, struct msghdr *msg,
...
static __always_inline int unix_bpf_recvmsg(struct sock *sk, struct msghdr *msg,
...
static __always_inline void unix_dgram_bpf_rebuild_protos(struct proto *prot, const struct proto *base)
{
	*prot        = *base;
...
static __always_inline void unix_stream_bpf_rebuild_protos(struct proto *prot,
...
static __always_inline void unix_dgram_bpf_check_needs_rebuild(struct proto *ops)
{
...
static __always_inline void unix_stream_bpf_check_needs_rebuild(struct proto *ops)
{
...
__always_inline int unix_dgram_bpf_update_proto(struct sock *sk, struct sk_psock *psock, bool restore)
{
	if (sk->sk_type != SOCK_DGRAM)
		return -EOPNOTSUPP;
...
__always_inline int unix_stream_bpf_update_proto(struct sock *sk, struct sk_psock *psock, bool restore)
{
	if (restore) {
		sk->sk_write_space = psock->saved_write_space;
		WRITE_ONCE(sk->sk_prot, psock->sk_proto);
...
__always_inline void __init unix_bpf_build_proto(void)
{
