...
func DoQueryCpuUtilize() (res []client.Result, err error) {
	res, err = influxdb.QueryDB(`select "perce" from "cpuutilize" where time > now() - 7d`)
	if err != nil {
		zap.L().Error("ERROR in DoQueryIRQ():", zap.Error(err))
		return nil, err
	}
...
func DoQueryPickNext() (res []client.Result, err error) {
	res, err = influxdb.QueryDB(`select "duration" from "picknext" where time > now() - 7d`)
	if err != nil {
...
func DoQueryTaskSwitch() (res []client.Result, err error) {
	res, err = influxdb.QueryDB(`select "duration" from "taskswitch" where time > now() - 7d`)
	if err != nil {
...
func DoQueryHardDiskReadWriteTime() (res []client.Result, err error) {
	res, err = influxdb.QueryDB(`select "lat" from "HardDiskReadWriteTime" where time > now() - 7d`)
	if err != nil {
