...
data_struct = {"measurement":'filelife',
               "time":[],
               "tags":['glob',],
               "fields":['time','pid','comm','age_seconds','file']}
...
def print_event(cpu, data, size):
    ...
    # write to influxdb
    test_data = lmp_data(strftime("%H:%M:%S"),'glob',event.pid,event.comm.decode('utf-8', 'replace'),float(event.delta) / 1000,event.fname.decode('utf-8', 'replace'))
    #print(test_data)
